      ...
      if (s != "") {
		...
		for (int i=0; i<sensor_fusion.size(); i++) {
		  ...
		  check_car_s += ((double)prev_size*.02*check_speed);
                  ...
                  // check if the lane change is needed and possible
		  if((other_car_lane == lane) && (check_car_s > car_s) && ((check_car_s - car_s) < 30)) {
		    too_close = true;
		  } 
		  if((move_left) && ((lane-1) == other_car_lane) && fabs(check_car_s - car_s) < 30) {
		    move_left = false;
		  }
		  if((move_right) && ((lane+1) == other_car_lane) && fabs(check_car_s - car_s) < 30) {
		    move_right = false;
		  }
		}

		// Behaviour - lane change logic (if needed and possible)
		if(too_close) 
		{
	          if (move_left) {
		    lane -= 1;
		...
		/*
		 * Trajectory Generation - once we have predicted the vehicle positions and determined the behaviour, next step is to generate trajectory
		 */

	        // create a list of widely spaced (x,y) waypoints, evenly spaced at 30m
		// later we will interpolate these waypoints with spline and fill it in with more points that control speed
		vector<double> ptsx;
		vector<double> ptsy;
		...
		if (prev_size < 2) {
		  ...
		  //use two points that make the point tangent to the previous path's end point
		...
		// start with all the previous paths points from last time	
